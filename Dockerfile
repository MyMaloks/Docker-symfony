...
RUN apt-get update -qy && \
    apt-get install -qq --no-install-recommends wget apt-transport-https lsb-release ca-certificates yamllint rsync npm git ssh unzip && \			
	...
	chmod a+x /usr/local/bin/php-cs-fixer-v2 && \
	npm install -g grunt 
